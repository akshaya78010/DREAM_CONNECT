...
const getMyDreams = async (req, res) => {
  try {
    const dreams = await Dream.find({ user: req.user._id }).sort({
      createdAt: -1,
    });
    res.json(dreams);
  } catch (error) {
...
const createDream = async (req, res) => {
  try {
    const { title, description, category, location, isPrivate, milestones } =
      req.body;
    let mediaUrl = "";
    let mediaType = "none";
    let media = [];
...
const likeDream = async (req, res) => {
  try {
    ...
    if (isLiked) {
      dream.likes = dream.likes.filter(
...
const addComment = async (req, res) => {
  try {
    ...
    const populatedDream = await Dream.findById(req.params.id).populate(
      "comments.user",
      "username",
    );
