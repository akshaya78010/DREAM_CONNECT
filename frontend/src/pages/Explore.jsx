...
export default function Explore() {
  ...
  const handleLike = async (dreamId) => {
    ...
    try {
      ...
      setDreams(
        dreams.map((d) => (d._id === dreamId ? { ...d, likes: res.data } : d)),
      );
    } catch (err) {
      console.error(err);
    }
  ...
  const submitComment = async (dreamId) => {
    ...
    try {
      ...
      setDreams(
        dreams.map((d) =>
          d._id === dreamId ? { ...d, comments: res.data } : d,
        ),
      );
    ...
    >
      <div style={{ textAlign: "center", marginBottom: "1rem" }}>
        ...
        <p style={{ color: "var(--text-muted)" }}>
          Get inspired by what others are building towards
        </p>
      </div>
      ...
      >
        ...
        <button
          ...
          className={`btn ${filter !== "dreams" ? "btn-secondary" : ""}`}
        >
          Active Dreams
        </button>
        <button
          onClick={() => setFilter("achievements")}
          className={`btn ${filter !== "achievements" ? "btn-secondary" : ""}`}
        >
          Achievements
        </button>
      </div>
        ...
        {renderedDreams.map((dream) => (
          ...
          >
            ...
            <h3
              style={{
                margin: "0.5rem 0 0 0",
                color: dream.isFulfilled ? "#10b981" : "var(--primary)",
                fontSize: "1.4rem",
              }}
            >
              {dream.title}
            ...
            >
              <span
                ...
                style={{
                  ...
                  alignItems: "center",
                  gap: "0.4rem",
                  color: dream.likes?.includes(user?._id)
                    ? "#ec4899"
                    : "var(--text-muted)",
                }}
              >
                {dream.likes?.includes(user?._id) ? "❤️" : "🤍"}{" "}
                {dream.likes?.length || 0}
              </span>
              <span
                onClick={() =>
                  setActiveCommentSection(
                    activeCommentSection === dream._id ? null : dream._id,
                  )
                }
                style={{
                  cursor: "pointer",
                ...
                {user && (
                  <div style={{ display: "flex", gap: "0.5rem" }}>
                    ...
                    <button
                      ...
                      onClick={() => submitComment(dream._id)}
                    >
                      Send
                    </button>
                  </div>
