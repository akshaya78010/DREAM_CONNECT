        ...
        {renderedDreams.map(dream => (
          <div key={dream._id} className="glass" style={{padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem', border: dream.isFulfilled ? '1px solid #10b981' : ''}}>
            ...
            {dream.media && dream.media.length > 0 ? (
              ...
              <>
                {dream.mediaType === 'video' && <video src={dream.mediaUrl?.startsWith('http') ? dream.mediaUrl : `${API_BASE_URL}${dream.mediaUrl}`} controls autoPlay muted loop style={{width: '100%', borderRadius: '8px', marginTop: '0.5rem', maxHeight: '500px', backgroundColor: '#000'}} />}
                ...
                {dream.mediaType === 'image' && <img src={dream.mediaUrl?.startsWith('http') ? dream.mediaUrl : `${API_BASE_URL}${dream.mediaUrl}`} alt="Dream media" style={{width: '100%', borderRadius: '8px', marginTop: '0.5rem', maxHeight: '500px', objectFit: 'contain', backgroundColor: 'rgba(0,0,0,0.2)'}} />}
              </>
            )}
