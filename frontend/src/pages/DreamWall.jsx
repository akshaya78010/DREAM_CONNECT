...
export default function DreamWall() {
  ...
  const markFulfilled = async (id) => {
    if (
      !window.confirm(
        "Are you sure you want to mark this dream as successfully achieved?",
      )
    )
      return;
    try {
      await axios.put(
        `/api/dreams/${id}/fulfill`,
        {},
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        },
  ...
  const progressPercent =
    totalDreams === 0 ? 0 : Math.round((achievedDreams / totalDreams) * 100);
          ...
          {dreams.map((dream) => (
            ...
            >
              ...
              >
                ...
                {!dream.isFulfilled && (
                  <button
                    onClick={() => markFulfilled(dream._id)}
